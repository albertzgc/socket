...
int main(int argc, const char* argv[]) {
	...
	if ((udp_sockfd = socket(PF_INET, SOCK_DGRAM, 0)) == -1) {
		cout << "Server A failed to create UDP socket" << endl;
		exit(1);
	}
	memset(&udp_server_addr, 0, sizeof udp_server_addr);
	udp_server_addr.sin_family = AF_INET;
	udp_server_addr.sin_port = htons(SERVER_A_UDP);
	...
	cout << "The Server A is up and running using UDP on port " << SERVER_A_UDP << endl;
	...
	while(1) {
		...
		if ((numbytes = recvfrom(udp_sockfd,aws_req_buf, sizeof aws_req_buf,0,
			...
			cout << "Server A failed to receive data from AWS" << endl;
			exit(1);
		}
		...
		while (maps[index].map_id != id) {
			...
			if(index == 52){
				...
				if (sendto(udp_sockfd, graph_not_found_message.c_str(), sizeof graph_not_found_message, 0, 
					...
					cout << "Server A failed to send error message to AWS" << endl;
					exit(1);
				}
